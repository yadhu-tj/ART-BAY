...
from models.artist_dashboard_queries import (
    ...
    add_artwork as db_add_artwork,
    delete_artwork_for_artist,
    update_artwork_details
)
from models.art_queries import get_art_by_id
...
def dashboard_home():
    ...
    try:
        ...
        artworks = get_artworks_by_artist(email)
        return jsonify({
            "status": "success",
            "artworks": artworks,
            "dashboard": True
...
def add_artwork():
    ...
    try:
        ...
        # NOTE: db_add_artwork returns {"message": ...}, doesn't return ID directly.
        # We might not be able to fetch the new artwork immediately without an ID.
        # For now, returning success message.
        return jsonify({
            "status": "success",
            "message": "Artwork added successfully",
            # "artwork": artwork # Cannot return artwork object without ID
        }), 201
    except Exception as e:
...
def delete_artwork(art_id):
    ...
    try:
        ...
        if artwork["email"] != email:
            return jsonify({"status": "error", "message": "Unauthorized to delete this artwork"}), 403
        
        # Delete from database
        success = delete_artwork_for_artist(art_id, email)
        
        if not success:
            return jsonify({"status": "error", "message": "Failed to delete from DB"}), 500
...
def update_artwork(art_id):
    ...
    try:
        ...
        # Note: Image update logic is missing in the query function 'update_artwork_details', 
        # so we are skipping image update in DB for now as per available function.
        # Ideally, we should update the query to support image path updates.

        # Update in database
        success = update_artwork_details(
            art_id=art_id,
            artist_email=email,
            title=title,
            description=description,
        ...
        if not success:
            return jsonify({"status": "error", "message": "Failed to update artwork in DB"}), 500
        
        # Get updated artwork details
        updated_artwork = get_art_by_id(art_id)
